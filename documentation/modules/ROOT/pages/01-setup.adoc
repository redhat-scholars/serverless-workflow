...
If you had no time to prepare upfront your environment, the lab instructors will provide you an USB Stick to significantly reduce the network usage during the lab execution:
...
4. Install the Visual Studio Code using the provided the binaries in the `swf-lab` folder
...
| `podman`
3+| Check detailed instruction *below* under the proper tab
...
NOTE: The parameter `--platform linux/amd64` is important when you run it on a Mac Silicon M1 (which has a different processor architecture)
