...
* *Pure Cloud*: using https://start.kubesmarts.org/dev[Serverless Logic Web Tools] and OpenShift as the runtime engine.
* *Browser with local runtime*: using https://start.kubesmarts.org/dev[Serverless Logic Web Tools] and `kn-workflow` command line (which leverages a local container engine, e.g. `podman`, `docker`)
