...
One important aspect is the _correlation definition_ which says that _CloudEvent_ has to carry a special *attribute in the header* named `orderid`.
...
* set `end` to `true`, in order to connect it to the end state.
